  ...
  describe('文件扫描性能测试', () => {
    ...
    test('扫描混合类型文件（JS/TS/Vue）应在合理时间内完成', async () => {
      ...
      for (let i = 0; i < vueCount; i++) {
        fs.writeFileSync(
          path.join(testDir, `Component${i}.vue`),
          generateVueContent(`Comp${i}`, 'medium')
        );
      }
  ...
  describe('性能回归检测', () => {
    test('基准性能指标应在预期范围内', () => {
      ...
      for (const [name, data] of Object.entries(benchmarks)) {
        ...
        console.log(
          `   ${name.padEnd(12)} ${data.threshold.toString().padStart(8)} ${data.actual.toFixed(3).padStart(10)}    ${status}`
        );
